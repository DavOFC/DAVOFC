let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        ...
        await m.reply('Byee👋, Bot akan keluar dari group, ownernya gak asik soalnya 🗿', args[0])
        await conn.groupLeave(args[0])
        await m.reply('Succes!!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply('Byee👋, Bot akan keluar dari group, ownernya gak asik soalnya 🗿', m.chat) // WKWKW pesannya sama semua. gk kreatif :v
